...
RSpec.describe 'Purchase History', type: :request do
  ...
  describe 'GET /purchases' do
    context 'when the buyer has past purchases' do
      let(:buyer)  { User.create!(email: 'buyer@example.com', role: 'buyer', password: 'password123') }
      let(:seller) { User.create!(email: 'seller@example.com', role: 'seller', password: 'password123') }
    ...
    context 'when the buyer has no purchases' do
      let(:buyer) { User.create!(email: 'newbuyer@example.com', role: 'buyer', password: 'password123') }
